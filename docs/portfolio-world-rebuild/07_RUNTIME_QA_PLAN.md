...
Status: **PROPOSED — plan only, harness not implemented**
Updated: 2026-09-26
...
Owned by `tools/agent-skills/profiles/portfolio-world-visual-qa/SKILL.md`, applied here to the specific blockout scope from `05_HARBOR_BLOCKOUT_SPEC.md` §7.
...
| B | Whole blockout overview | The full crescent-harbor composition in one frame — the "does the harbor read immediately" test |
| C | Hero Ship + waterfront framing | The diagonal Square→pier sightline from `05_HARBOR_BLOCKOUT_SPEC.md` §4 |
...
### Blockout PASS conditions (qualitative judgment against `00_VISUAL_BRIEF.md`, no final-material judgment yet)

```text
- harbor reads immediately (screenshot B)
- Hero Ship reads as Tier 1 (screenshot C)
- layout is NOT a cardinal cross (screenshot B)
- water has meaningful negative space (screenshots B, C)
- the one included destination (Exhibition Hall) remains easy to locate (screenshot A)
- scale feels plausible (screenshot D, judged against the four calibration
  questions in 06_SCALE_CAMERA_CALIBRATION_PLAN.md §2)
```

None of these six conditions may be granted from a code value, a passing functional harness run (§1), a pixel-diff, or an unopened screenshot — consistent with `00_VISUAL_BRIEF.md` §13.3 and `portfolio-world-visual-qa`'s own rule. **Final-material judgment (palette, texture density, lighting) is explicitly out of scope for this PASS** — the blockout uses flat shapes/placeholder blocks by design (`05_HARBOR_BLOCKOUT_SPEC.md` §7), and judging material quality on placeholder shapes would be a category error, not rigor.
