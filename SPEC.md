...
- 배포: GitHub Pages
- 하위 페이지: `career.html`(1호), `teaching.html`(2호 — 강사 페이지. 스펙은 `SPEC_teaching.md`), `gallery.html`(3호 — 추가 예정, 스펙은 `SPEC_gallery.md`)
...
| 홈 섹션 앵커 | 소개·역량·프로젝트·문의 4개를 GNB 보조 그룹(작고 옅은 스타일)으로 노출한다. 모바일 햄버거 메뉴에서는 독립 페이지 그룹 아래에 구분선으로 나눠 노출한다. |
